...
автоматически: район (`d<id>`) → регион. S2 и L8 объединяются в один
ряд: при совпадении периода предпочитается Sentinel-2, пропуски
закрываются Landsat; у каждого фрейма свой `sensor`.
...
```json
{
  ...
  "scope": "d5",
  "frames": [
    {"date_from": "2025-07-01", "date_to": "2025-07-05", "date": "2025-07-01_2025-07-05", "sensor": "s2"}
  ]
}
```
...
| Весы | `valid_pixel_count / pixel_count` | Доля чистого покрытия поля каждым сенсором — не зависит от разрешения (абсолютный счёт пикселей S2 10м ≫ L 30м и глушил бы вклад Landsat) |
