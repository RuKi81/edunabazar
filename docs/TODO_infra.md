...
## [x] DONE: CI Test job медленный (~20 мин)

**Закрыто (июль 2026):** после чистки CI (apt-кэш GDAL/GEOS, pip-кэш,
`--parallel auto`, удаление дублирующего `migrate`-шага) job проходит за
~1.5–2 мин: 129 тестов за ~5 с на 5 воркерах, весь test-шаг ~13 с.
Таймаут возвращён 30 → 10 мин. В test-шаге оставлены `time` и
`--durations 20` — постоянная видимость деградаций.
...
## [ ] РЕШЕНИЕ: рост `DistrictNdviSeries` (сейчас ~1.17M строк)

Таблица `agro_district_ndvi_series` пополняется ежедневным мониторингом
всех ~2200 районов (id уже мигрирован на bigint в `0034`). Ретеншн не
подходит: серии за прошлые годы нужны для baseline-сравнений. Агрегация
не подходит: MODIS и так 8-дневный шаг.

**Решение (июль 2026): пока НЕ трогать.** Хот-пас покрыт составным
индексом (`Meta.indexes`), unique_together в наличии, при текущем темпе
(~2200 районов × ~45 композитов/год × культуры) до проблемных объёмов
годы.

**Триггер для действия:** > 10M строк ИЛИ деградация
`/api/ndvi-stats/?region=…` (p95 > 1 c). Тогда — нативное
партиционирование PostgreSQL `PARTITION BY RANGE (acquired_date)` по
годам: ручная миграция + krontab/pg_partman на создание партиции
следующего года. Django с партиционированной таблицей работает
прозрачно (те же запросы), не умеет только создавать партиции сам.
...
## [ ] TODO: задеплоить фикс 504 на MVT-тайлах ЗСН (готов локально, НЕ запушен)

**Статус (3 сен 2026): код написан и протестирован локально, деплой отложен по решению пользователя.**

**Симптом:** карта на `/me/gis` падала с
`AJAXError: (504): https://edunabazar.ru/agrocosmos/api/tiles/6/41/20.pbf?v=...`.

**Корневая причина:** источник ЗСН в `my_fields/templates/my_fields/gis.html`
запрашивает тайлы БЕЗ фильтра `region`/`district` (общероссийская выборка)
и с `minzoom: 6`. На `z=6` bbox тайла охватывает огромную часть страны —
`ST_AsMVT` по миллионам полигонов `agro_farmland` не укладывается в таймаут
шлюза → 504. Дашборды (`dashboard.html`, `raster_dashboard.html`) не ловят
баг, т.к. всегда шлют `?region=`/`?district=` (ограничено индексом FK).

**Что уже сделано локально (НЕ закоммичено/не запушено):**
- `agrocosmos/views/tiles.py`: пороги `MIN_TILE_ZOOM_FILTERED = 5` и
  `MIN_TILE_ZOOM_UNFILTERED = 11`; в `_api_tile_cached` для `z` ниже порога
  сразу отдаётся пустой кэшируемый тайл БЕЗ обращения к БД (нефильтрованный
  порог выше, т.к. запрос не ограничен FK).
- `my_fields/templates/my_fields/gis.html`: `minzoom` источника `farmlands`
  поднят `6 → 11` (согласовано с серверным порогом; JS инлайновый в шаблоне).
- `agrocosmos/tests/test_tiles.py`: регрессия (4 теста, зелёные, flake8 чист) —
  нефильтрованный низкий зум = пустой тайл без SQL, фильтрованный `z=6` уходит
  в БД, не-админ = 403 без запроса.

**Что осталось:**
- [ ] `git push origin main` → CI автодеплой.
- [ ] После деплоя: жёсткое обновление (Ctrl+F5) — JS инлайновый в шаблоне.
- [ ] Проверить, что серверный кэш пустых 504-тайлов истёк (TTL 10 мин) или
  дождаться нового `?v=` при перезагрузке страницы.

**Побочный эффект UX:** слой ЗСН на `/me/gis` появляется теперь с `z=11`
(на масштабе страны/региона отдельные контуры субпиксельны). Дашборды с
фильтром по региону/району работают как прежде.

**Приоритет:** высокий (публичная карта отдаёт 504).
