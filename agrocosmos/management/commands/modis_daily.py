"""
Download MODIS daily NDVI composites (MOD09GQ+MYD09GQ) and compute zonal stats.

Uses daily surface reflectance (1-2 day publication lag) instead of
MOD13Q1 16-day composites (2-3 week lag) for near-real-time monitoring.

NDVI is computed from Red/NIR bands with QC-based cloud masking.
5-day median composites reduce noise while maintaining high frequency.

Usage:
    # Full cycle: download + compute for Crimea 2025
    python manage.py modis_daily --region-id 37 --year 2025

    # Custom date range
    python manage.py modis_daily --region-id 37 \
        --date-from 2025-03-01 --date-to 2025-10-31

    # Only download rasters
    python manage.py modis_daily --region-id 37 --year 2025 --download-only

    # Only compute stats (rasters already downloaded)
    python manage.py modis_daily --region-id 37 --year 2025 --stats-only

NOTE: Old MOD13Q1 pipeline (modis_ndvi command) is kept intact for rollback.
"""
import json
import signal
import time
...
class Command(BaseCommand):
    help = 'Download MODIS daily NDVI (MOD09GQ+MYD09GQ) and compute zonal stats locally'
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--min-valid-ratio', type=float, default=0.3,
                            help='Min valid pixel ratio (default: 0.3, lower than MOD13Q1)')
        parser.add_argument('--overwrite', action='store_true',
                            help='Re-download existing rasters')

    def handle(self, *args, **options):
        from agrocosmos.services.satellite_modis_daily import (
            compute_zonal_stats, download_year, _daily_chunks, _raster_path,
        )
        ...
        self.stdout.write(
            f'═══════════════════════════════════════════════\n'
            f'  MODIS Daily NDVI — MOD09GQ+MYD09GQ Pipeline\n'
        ...
        if not stats_only:
            self.stdout.write('\n📡 Step 1: Downloading MODIS daily composites from GEE…')
            downloaded = 0
            skipped = 0
            errors = 0
            ...
            for i, (cf, ct) in enumerate(chunks):
                ...
                try:
                    from agrocosmos.services.satellite_modis_daily import download_composite
        ...
        for i, (cf, ct) in enumerate(chunks):
            ...
            for did in district_ids_needed:
                scene_id = f'modis_daily_{mid_date.isoformat()}_{did}'
                scene, _ = SatelliteScene.objects.get_or_create(
                    scene_id=scene_id,
                    defaults={
                        'satellite': 'modis_daily',
            ...
            # Upsert via INSERT ... ON CONFLICT UPDATE
